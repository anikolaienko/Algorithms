...
TEST(TicTacToeTest, InitializeBoard) {
    ...
    EXPECT_FALSE(board.is_finished(), false);
...
TEST(TicTacToeTest, InitializeBoardWithWinner) {
    std::string str_board = "xx_" + "\n"
                          ...
                          + "_xo";
    Algorithms::TicTacToe board(str_board);
    EXPECT_FALSE(board.is_finished(), true);
...
TEST(TicTacToeTest, InitializeWithWrongBoardSize) {
    std::string str_board = "oxo_" + "\n"
                          ...
                          + "xoo_";
    Algorithms::TicTacToe* board;
    EXPECT_THROW(new Algorithms::TicTacToe(str_board), invalid_argument);
}
...
TEST(TicTacToeTest, MoveWithWrongCoordinate) {
    Algorithms::TicTacToe board();
    EXPECT_THROW(board.move(-1,0,'x'), invalid_argument);
    EXPECT_THROW(board.move(0,-1,'x'), invalid_argument);
    EXPECT_THROW(board.move(3,0,'o'), invalid_argument);
    EXPECT_THROW(board.move(0,3,'o'), invalid_argument);
    EXPECT_THROW(board.move(10,23,'o'), invalid_argument);
}

TEST(TicTacToeTest, MoveWithWrongSymbol) {
    Algorithms::TicTacToe board();
    EXPECT_THROW(board.move(-1,0,'_'), invalid_argument);
    EXPECT_THROW(board.move(-1,0,'a'), invalid_argument);
}